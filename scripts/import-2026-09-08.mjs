...
const SOURCE = 'MANUAL_IMPORT_2026-09-08'
const accountId = process.env.CLOUDFLARE_ACCOUNT_ID
const token = process.env.CLOUDFLARE_API_TOKEN
if (!accountId || !token) throw new Error('Missing Cloudflare credentials')

const receipts = {
  '0-0030': { total: 65, receipt_date: '2026-09-08T11:18:42.000Z' },
  '0-0031': { total: 120, receipt_date: '2026-09-08T11:54:07.000Z' },
  '0-0032': { total: 100, receipt_date: '2026-09-08T12:26:31.000Z' },
  '0-0033': { total: 45, receipt_date: '2026-09-08T13:11:52.000Z' },
  '0-0034': { total: 65, receipt_date: '2026-09-08T14:03:19.000Z' },
  '0-0035': { total: 150, receipt_date: '2026-09-08T14:47:44.000Z' },
  '0-0036': { total: 65, receipt_date: '2026-09-08T15:32:08.000Z' },
}

const lines = [
  { lp: 1, receipt: '0-0030', price: 40, delivery: '0', item: 'Torebka', sku: '10005' },
  { lp: 2, receipt: '0-0030', price: 15, delivery: '0', item: 'Pasek', sku: '10006' },
  { lp: 3, receipt: '0-0030', price: 10, delivery: '0', item: 'T-shirt', sku: '10024' },

  { lp: 4, receipt: '0-0031', price: 30, delivery: '2', item: 'Shorty', sku: '10037' },
  { lp: 5, receipt: '0-0031', price: 45, delivery: '0', item: 'Spodnie', sku: '10041' },
  { lp: 6, receipt: '0-0031', price: 45, delivery: '1', item: 'Bluzka', sku: '10027' },

  { lp: 7, receipt: '0-0032', price: 45, delivery: '1', item: 'Bluzka', sku: '10027' },
  { lp: 8, receipt: '0-0032', price: 55, delivery: '0', item: 'Koszula', sku: '10026' },

  { lp: 9, receipt: '0-0033', price: 40, delivery: '2', item: 'Bluza', sku: '10044' },
  { lp: 10, receipt: '0-0033', price: 5, delivery: '2', item: 'Bluzka', sku: '10027' },

  { lp: 11, receipt: '0-0034', price: 40, delivery: '1', item: 'Spodnie', sku: '10041' },
  { lp: 12, receipt: '0-0034', price: 25, delivery: '2', item: 'Koszula', sku: '10026' },

  { lp: 13, receipt: '0-0035', price: 150, delivery: '2', item: 'Sneakersy', sku: '10018' },
  { lp: 14, receipt: '0-0036', price: 65, delivery: '1', item: 'Stanik', sku: '10032' },
]
...
function lineId(lp) {
  return `manual-20260908-${String(lp).padStart(4, '0')}`
}

function paymentKey(receiptNumber) {
  return `manual-20260908-payment-${receiptNumber}`
}

function validatePlan() {
  const receiptNumbers = Object.keys(receipts)
  assert(receiptNumbers.length === 7, `Expected 7 receipts in plan, got ${receiptNumbers.length}`)
  assert(lines.length === 14, `Expected 14 lines in plan, got ${lines.length}`)

  const receiptTotal = Object.values(receipts).reduce((sum, r) => sum + r.total, 0)
  const linesTotal = lines.reduce((sum, l) => sum + l.price, 0)
  assert(receiptTotal === 610, `Planned receipt total mismatch: ${receiptTotal}`)
  assert(linesTotal === 610, `Planned line total mismatch: ${linesTotal}`)
...
async function rollback() {
  ...
  await query(`DELETE FROM receipt_payments WHERE payment_key LIKE 'manual-20260908-payment-%'`)
  await query(`DELETE FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%'`)
...
async function verify() {
  const summary = (await query(
    `SELECT
       ...
       (SELECT COUNT(*) FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%') AS lines_count,
       (SELECT ROUND(SUM(total_money),2) FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%') AS lines_total,
       (SELECT COUNT(*) FROM receipt_payments WHERE payment_key LIKE 'manual-20260908-payment-%') AS payments_count,
       (SELECT ROUND(SUM(money_amount),2) FROM receipt_payments WHERE payment_key LIKE 'manual-20260908-payment-%') AS payments_total`,
    [SOURCE, SOURCE],
  ))[0]

  assert(Number(summary.receipts_count) === 7, `Expected 7 receipts, got ${summary.receipts_count}`)
  assert(Number(summary.lines_count) === 14, `Expected 14 lines, got ${summary.lines_count}`)
  assert(Number(summary.payments_count) === 7, `Expected 7 payments, got ${summary.payments_count}`)
  assert(Number(summary.receipts_total) === 610, `Receipt total mismatch: ${summary.receipts_total}`)
  assert(Number(summary.lines_total) === 610, `Line total mismatch: ${summary.lines_total}`)
  assert(Number(summary.payments_total) === 610, `Payment total mismatch: ${summary.payments_total}`)

  const detail = (await query(
    `SELECT
       SUM(CASE WHEN total_discount=0 THEN 0 ELSE 1 END) AS bad_receipt_discount,
       (SELECT SUM(CASE WHEN total_discount=0 THEN 0 ELSE 1 END) FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%') AS bad_line_discount,
       (SELECT SUM(CASE WHEN cost=0 AND cost_total=0 THEN 0 ELSE 1 END) FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%') AS bad_cost,
       (SELECT SUM(CASE WHEN item_id IS NOT NULL THEN 0 ELSE 1 END) FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%') AS missing_item_id,
       (SELECT SUM(CASE WHEN sku IS NOT NULL AND trim(sku)<>'' THEN 0 ELSE 1 END) FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%') AS missing_sku,
       (SELECT SUM(CASE WHEN line_note IS NOT NULL AND trim(line_note)<>'' AND trim(line_note)<>'-1' THEN 0 ELSE 1 END) FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%') AS bad_line_note
  ...
  const itemMismatch = (await query(
    `SELECT COUNT(*) AS n
     ...
     WHERE l.line_id LIKE 'manual-20260908-%'
    ...
    `SELECT COUNT(*) AS n
     ...
     WHERE l.line_id LIKE 'manual-20260908-%'
       AND NOT EXISTS (
         ...
         WHERE h.item_id=l.item_id AND h.sku=l.sku AND h.line_id NOT LIKE 'manual-20260908-%'
       )`,
  ))[0]
  ...
  const times = (await query(
    `SELECT COUNT(*) AS n FROM receipts
     WHERE source=? AND (
       date(receipt_date,'+2 hours')<>'2026-09-08'
       OR time(receipt_date,'+2 hours')<'13:00:00'
       OR time(receipt_date,'+2 hours')>='18:00:00'
     )`,
    [SOURCE],
  ))[0]
  assert(Number(times.n) === 0, `Receipt date/time outside 08.09.2026 13:00-18:00 PL: ${times.n}`)

  const paymentShape = (await query(
    `SELECT COUNT(*) AS n FROM receipt_payments
     WHERE payment_key LIKE 'manual-20260908-payment-%'
  ...
  const paymentsSummary = await query(
    `SELECT name,COUNT(*) AS receipts,ROUND(SUM(money_amount),2) AS amount
     FROM receipt_payments WHERE payment_key LIKE 'manual-20260908-payment-%'
     GROUP BY name ORDER BY name`,
  )

  const deliverySummary = await query(
    `SELECT line_note,COUNT(*) AS lines,ROUND(SUM(total_money),2) AS amount
     FROM receipt_lines WHERE line_id LIKE 'manual-20260908-%'
